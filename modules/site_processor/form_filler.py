...
from modules.site_processor import phone_formatter
            ...
            try:
                ...
                for element in elements:
                    if regex.search(element.get_attribute(attr) or ''):
                        if field_type == 'phone':
                            phone = phone_formatter.run(driver)
                            self.config['form_data'][field_type] = self.process_phone_number(driver)
                        element.clear()
                        element.send_keys(self.config['form_data'][field_type])
